				...
				<section class="boxs">
					<div class="boxs-header">
						<h3 class="custom-font hb-blush">
							<strong>Update</strong> Area
						</h3>
					</div>
					<div class="boxs-body">
						<form class="form-horizontal" name="form4" action="<?php echo site_url(); ?>/AreaController/cupdate/<?php echo $editData->area_id ?>" method="post" role="form" id="form4" data-parsley-validate>
							<div class="form-group">
								<label class="col-sm-3 control-label">State Name</label>
								<div class="col-sm-9">
									<select class="form-control" name="sid" onchange="getCity(this.value)">
										<!-- <option>Select State Name</option> -->
										<?php foreach($stateData as $row) { 
											
											if($editData->state_id == $row->state_id)
											{
											?>
								...
								<div class="col-sm-9">
									<select id="city" class="form-control" name="cid">
										...
										<?php foreach($cityData as $row) {
											...
											if($editData->city_id == $row->city_id)
											{
											?>
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<input type="text" class="form-control" name="areaName" value="<?php echo $editData->area_name ?>" placeholder="Area Name" data-parsley-trigger="change" required>
								</div>
							</div>
					</div>
...
<script>
	function getCity(stateId){
		$.ajax({
			url:"<?php echo site_url(); ?>/AreaController/getState/"+stateId,
			method:"post",
